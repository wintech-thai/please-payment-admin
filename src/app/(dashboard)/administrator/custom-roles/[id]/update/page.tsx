...
import type { CustomRoleItem, UpdateCustomRolePayload, ControllerPermissions } from '@/lib/api/types'
...
export default function UpdateCustomRolePage() {
  ...
  const [role, setRole] = useState<CustomRoleItem | null>(null)
  ...
  useEffect(() => {
    Promise.all([
      customRoleApi.getCustomRoleById(id),
      customRoleApi.getInitialUserRolePermissions(),
    ])
      .then(([roleRes, permsRes]) => {
        const r: CustomRoleItem = roleRes.data.customRole ?? roleRes.data
        setRole(r)
        setName(r.roleName)
        setDesc(r.roleDescription ?? '')
        // Parse comma-separated tags string into chip array
        ...
        const initial = permsRes.data.permissions ?? []
        setPermGroups(existing?.length ? existing : initial)
      })
      .catch(() => {
        toast.error(t.customRoles.failedToLoad2)
        router.push('/administrator/custom-roles')
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: UpdateCustomRolePayload = {
        roleName: name.trim(),
        roleDescription: desc.trim() || undefined,
        tags: finalTags.length ? finalTags.join(',') : undefined,
        permissions: permGroups,
      }
      await customRoleApi.updateCustomRoleById(id, payload)
      setIsDirty(false)
      toast.success(t.customRoles.updatedSuccess)
      router.push(`/administrator/custom-roles?highlight=${id}`)
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : t.customRoles.failedToUpdate
      toast.error(msg)
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center gap-3 mb-6">
        <button
          onClick={() => guardNavigation(() => router.push(`/administrator/custom-roles?highlight=${id}`))}
          className="p-2 rounded-lg text-gray-500 hover:bg-gray-200 transition-colors"
        >
          <ChevronLeft className="w-5 h-5" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t.customRoles.editTitle}</h1>
          <p className="text-base text-gray-500 mt-0.5">{t.customRoles.editSubtitle} &ldquo;{role?.roleName}&rdquo;</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-7 py-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            ...
            <FormField label={t.customRoles.fieldDescription}>
              <input
                value={desc}
        ...
        <div className="flex-none -mx-3 sm:-mx-6 px-4 sm:px-8 py-4 flex items-center justify-end gap-3 bg-white border-t border-gray-100 shadow-[0_-4px_12px_rgba(0,0,0,0.06)]">
          <button type="button" onClick={() => guardNavigation(() => router.push(`/administrator/custom-roles?highlight=${id}`))} className={cancelBtnCls}>
            {t.admin.cancel}
          </button>
