...
export default function UsersPage() {
  ...
  const fetchUsers = async (currentPage: number, keyword: string = '') => {
    ...
    try {
      const [usersRes, countRes] = await Promise.all([
        userApi.getUsers({ search: keyword || undefined, page: currentPage, limit: itemsPerPage }),
        userApi.getUserCount({ search: keyword || undefined }),
      ])
  ...
  const createUrl = selectedRowId
    ? `/administrator/users/invite?prevHighlight=${selectedRowId}`
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col flex-1 min-h-0">
        ...
        <div className="overflow-x-auto flex-1 min-h-0">
          <table className="min-w-full">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={9} className="py-20 text-center"><LoadingRow /></td></tr>
