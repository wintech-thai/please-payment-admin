...
export default function UpdateExpenseTypePage() {
  const { t } = useLang()
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        router.push('/business-setup/financial-settlement/expense-type')
      }
    }
    load()
  }, [itemId])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/business-setup/financial-settlement/expense-type?highlight=${itemId}`)
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : m.loadFailed)
    } finally {
      setSaving(false)
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center gap-3 mb-6">
        <button
          onClick={() => guardNavigation(() => router.push('/business-setup/financial-settlement/expense-type'))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5 max-w-2xl">
              ...
              <div className="sm:col-span-2">
                ...
                <div
                  ...
                  onClick={() => document.getElementById('expense-update-tag-input')?.focus()}
                >
                  {tags.map(tag => (
