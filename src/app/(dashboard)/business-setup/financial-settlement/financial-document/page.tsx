...
function FinancialDocListContent() {
  ...
  const [selectedRowId, setSelectedRowId] = useState<string | null>(() => {
    ...
    if (typeof window !== 'undefined') return sessionStorage.getItem('financial_doc_highlight') ?? null
    return null
  })
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; id?: string; name?: string }>({ open: false })
  const [deleting, setDeleting] = useState(false)

  const fetchData = async (p = page, search = searchTerm, tr = timeRange) => {
    setLoading(true)
    try {
      const { fromDate, toDate } = getTimeFilter(tr)
      const [listRes, countRes] = await Promise.allSettled([
        financialDocApi.getFinancialDocs({ FullTextSearch: search, FromDate: fromDate, ToDate: toDate, Page: p, Limit: itemsPerPage }),
        financialDocApi.getFinancialDocCount({ FullTextSearch: search, FromDate: fromDate, ToDate: toDate }),
      ])
  ...
  const handleSearch = () => { setPage(1); fetchData(1, searchTerm, timeRange) }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      document.getElementById(`financial-doc-row-${highlightIdParam}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 300)
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      await financialDocApi.deleteFinancialDocById(deleteModal.id)
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        <button
          onClick={() => router.push('/business-setup/financial-settlement/financial-document/create')}
      ...
      <div className="flex-none flex flex-wrap items-center gap-3 mb-4">
        <div className="flex items-center gap-2 flex-1 min-w-56 max-w-xs bg-white border border-gray-200 rounded-lg px-3 py-2 shadow-sm">
          ...
          />
          {searchTerm && (
            <button onClick={() => { setSearchTerm(''); fetchData(1, '', timeRange) }} className="text-gray-400 hover:text-gray-600">✕</button>
          )}
        </div>
        <AdvancedTimeRangeSelector
          value={timeRange}
          onChange={tr => { setTimeRange(tr); setPage(1); fetchData(1, searchTerm, tr) }}
        />
        <button
        ...
        <button
          onClick={() => {
            ...
            setDeleteModal({ open: true, id: selectedId ?? undefined, name: item?.documentNo ?? undefined })
          }}
          disabled={!selectedId}
          ...
          <table className="w-auto min-w-full text-sm border-separate border-spacing-0">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-50">
                {['', m.colCreatedDate, m.colDocumentNo, m.colDescription, m.colTags, m.colFromDate, m.colToDate, m.colAction].map((col, i) => (
                  <th
                    key={i}
                    className={clsx(
            ...
            <tbody>
              {loading ? (
                ...
                items.map((item, idx) => {
                  ...
                  return (
                    <tr
                      id={`financial-doc-row-${item.id}`}
                      key={item.id}
                      onClick={() => {
                        const next = selectedRowId === item.id ? null : item.id
                        setSelectedRowId(next)
                        if (next) sessionStorage.setItem('financial_doc_highlight', next)
                        else sessionStorage.removeItem('financial_doc_highlight')
                      }}
                      className={clsx(
