...
export default function CreateShareholderRatioPage() {
  const { t } = useLang()
  ...
  const [errors, setErrors] = useState<{ code?: string; shareholders?: string }>({})
  ...
  const validate = () => {
    const errs: { code?: string; shareholders?: string } = {}
    const trimmed = code.trim()
    if (!trimmed) errs.code = m.validationCodeRequired
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/business-setup/financial-settlement/shareholder-ratio${newId ? `?highlight=${newId}` : ''}`)
    } catch (err: unknown) {
      if (err instanceof AxiosError && err.code === 'DUPLICATE') {
        setErrors(p => ({ ...p, code: m.codeDuplicate }))
      } else {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5 max-w-2xl">
              ...
              <div className="sm:col-span-2">
                ...
                <div
                  ...
                  onClick={() => document.getElementById('shr-create-tag-input')?.focus()}
                >
                  {tags.map(tag => (
