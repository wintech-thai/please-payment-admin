...
function ShareholderRatioListContent() {
  const { t } = useLang()
  const m = t.shareholderRatio
  ...
  const [selectedRowId, setSelectedRowId] = useState<string | null>(() => {
    ...
    if (typeof window !== 'undefined') return sessionStorage.getItem('shareholder_ratio_highlight') ?? null
    return null
  })
  ...
  const fetchData = async (p = page, search = searchTerm) => {
    ...
    try {
      const [listRes, countRes] = await Promise.allSettled([
        masterRefApi.getMasterRefs({ RefType: 'ShareHolderRatio', FullTextSearch: search, Page: p, Limit: itemsPerPage }),
        masterRefApi.getMasterRefCount({ RefType: 'ShareHolderRatio', FullTextSearch: search }),
      ])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      document.getElementById(`shareholder-ratio-row-${highlightIdParam}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 300)
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        <button
          onClick={() => router.push('/business-setup/financial-settlement/shareholder-ratio/create')}
          ...
          <table className="w-auto min-w-full text-sm border-separate border-spacing-0">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-50">
                {['', m.colCode, m.colDescription, m.colTags, m.colShareholders, m.colAction].map((col, i) => (
                  <th
                    key={i}
                    className={clsx(
            ...
            <tbody>
              {loading ? (
                ...
                items.map((item, idx) => {
                  ...
                  const shareholders = item.definitionObj ?? []
                  return (
                    <tr
                      id={`shareholder-ratio-row-${item.id}`}
                      key={item.id}
                      onClick={() => {
                        const next = selectedRowId === item.id ? null : item.id
                        setSelectedRowId(next)
                        if (next) sessionStorage.setItem('shareholder_ratio_highlight', next)
                        else sessionStorage.removeItem('shareholder_ratio_highlight')
                      }}
                      className={clsx(
                    ...
                    >
                      ...
                      <td className="px-4 py-3 border-b border-gray-100 whitespace-nowrap">
                        <button
                          onClick={e => { e.stopPropagation(); router.push(`/business-setup/financial-settlement/shareholder-ratio/${item.id}/update`) }}
                      ...
                      <td className="px-4 py-3 border-b border-gray-100 max-w-[280px]">
                        <div className="flex flex-wrap gap-1">
                          {shareholders.map((s, i) => (
                            <span key={i} className="inline-flex px-2 py-0.5 bg-violet-50 text-violet-700 ring-1 ring-violet-200 rounded-full text-[10px] font-semibold">
                              {s.name} ({s.percent}%)
                            </span>
                          ))}
                        </div>
...
export default function ShareholderRatioPage() {
  return (
    <Suspense>
      <ShareholderRatioListContent />
    </Suspense>
  )
}
