...
const BASE_PATH = '/business-setup/master-reference/pay-in-approve-status'
const ROW_KEY = 'payin_approve_status_highlight'
...
function PayInApproveStatusListContent() {
  const { t } = useLang()
  const m = t.payInApproveStatus
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      document.getElementById(`payin-approve-row-${highlightIdParam}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 300)
...
export default function PayInApproveStatusPage() {
  return (
    <Suspense>
      <PayInApproveStatusListContent />
    </Suspense>
  )
}
