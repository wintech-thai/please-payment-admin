...
function PayOutBankAccountContent() {
  ...
  const [selectedRowId, setSelectedRowId] = useState<string | null>(() => {
    ...
    if (typeof window !== 'undefined') return sessionStorage.getItem('payoutbankaccount_highlight') ?? null
    return null
  })
  ...
  const fetchAccounts = async (currentPage: number, search = searchTerm, type = filterType, level = filterLevel) => {
    ...
    try {
      ...
      const [listRes, countRes] = await Promise.allSettled([
        bankAccountApi.getPayOutBankAccounts({ page: currentPage, limit: itemsPerPage, ...filterPayload }),
        bankAccountApi.getPayOutBankAccountCount({ ...filterPayload }),
      ])
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center justify-between mb-5">
        ...
        <Link
          href="/business-setup/pay-out-bank-account/create"
                  ...
                  return (
                    <tr
                      ...
                      onClick={() => {
                        ...
                        if (next) sessionStorage.setItem('payoutbankaccount_highlight', next)
                        else sessionStorage.removeItem('payoutbankaccount_highlight')
                      }}
                      className={clsx(
                    ...
                    >
                      ...
                      <td className="px-4 py-3 border-b border-gray-100 whitespace-nowrap">
                        <Link
                          href={`/business-setup/pay-out-bank-account/${account.accountId}/update`}
                      ...
                      <td className="px-4 py-3 border-b border-gray-100 text-sm text-gray-600 whitespace-nowrap">
                        {account.merchantLinkCount != null ? (
                          <button
                            onClick={e => { e.stopPropagation(); sessionStorage.setItem('payoutbankaccount_highlight', account.accountId); router.push(`/business-setup/pay-out-bank-account/${account.accountId}/merchant-link`) }}
                        ...
                        <div className="relative" ref={el => { menuRefs.current[account.accountId] = el }}>
                          ...
                          {openMenuId === account.accountId && (
                            <div className="fixed w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-[9999]"
                              ...
                              <button
                                onClick={e => { e.stopPropagation(); setOpenMenuId(null); sessionStorage.setItem('payoutbankaccount_highlight', account.accountId); router.push(`/business-setup/pay-out-bank-account/${account.accountId}/merchant-link`) }}
