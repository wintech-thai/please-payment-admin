...
import type { TransferRequestItem } from '@/lib/api/types'
  ...
  const load = useCallback(async (currentPage: number, limit: number, tr: TimeRangeValue, q: string, status: string) => {
    ...
    try {
      ...
      const [listRes, countRes] = await Promise.allSettled([
        paymentRequestApi.getTransferRequests(payload),
        paymentRequestApi.getTransferRequestCount({ ...payload }),
      ])

      if (listRes.status === 'rejected') throw listRes.reason

      const data = (listRes.value.data as any)
      const list: TransferRequestItem[] = Array.isArray(data)
        ? data
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center justify-between mb-5">
        ...
        <button
          onClick={() => router.push('/business-setup/payment/transfer-request/create')}
