...
// ── Page ─────────────────────────────────────────────────────────────────────
// เหมือนหน้า Add Pay-Out Request ทุกอย่าง ต่างกันแค่เรียก createWithdrawalRequest()
// แทน createPayOutRequest() (backend จะ set PayoutIsWithdrawal=true ให้เอง)
...
export default function CreateWithdrawalRequestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await paymentRequestApi.createWithdrawalRequest({
      ...
      setIsDirty(false)
      toast.success(m.toastCreateWithdrawSuccess)
      router.push('/business-setup/payment/withdraw-request')
      router.refresh()
    } catch (err: any) {
      toast.error(err?.message ?? m.toastCreateWithdrawFailed)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{m.createWithdrawTitle}</h1>
          <p className="text-sm text-gray-500 mt-0.5">{m.createWithdrawSubtitle}</p>
        </div>
      </div>
