...
export default function UpdateTransitBankAccountPage() {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      if (accRes.status === 'fulfilled') {
        ...
        router.push(`/business-setup/transit-bank-account?highlight=${id}`)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!isDirty) { router.push(`/business-setup/transit-bank-account?highlight=${id}`); return }
    setSaving(true)
    try {
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center gap-3 mb-6">
        <button
          onClick={() => guardNavigation(() => router.push(`/business-setup/transit-bank-account?highlight=${id}`))}
        ...
        <div className="flex-1 overflow-y-auto flex flex-col gap-4 pb-2 custom-scrollbar">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-7 py-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <FormField label={m.fieldAccountNumber}>
                <input
                  value={accountNumber}
                  readOnly
              ...
              <FormField label={m.fieldAccountName}>
                <input
                  value={accountName}
                  readOnly
                  className={clsx(inputCls(false), 'bg-gray-50 text-gray-500 cursor-not-allowed')}
              ...
              <FormField label={m.fieldPromptPayId}>
                <input
                  value={promptPayId}
                  readOnly
              ...
              <FormField label={m.fieldAccountLevel}>
                <input
                  value="Global"
                  readOnly
                  disabled
