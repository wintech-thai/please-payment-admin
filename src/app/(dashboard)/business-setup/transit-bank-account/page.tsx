...
import { Search, Plus, MoreHorizontal, Ban, CheckCircle, Landmark, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react'
...
function TransitBankAccountContent() {
  const { t } = useLang()
  const m = { ...t.bankAccount, ...t.transitBankAccount }
  ...
  const [selectedRowId, setSelectedRowId] = useState<string | null>(() => {
    ...
    if (typeof window !== 'undefined') return sessionStorage.getItem('transitbankaccount_highlight') ?? null
    return null
  })
  ...
  const fetchAccounts = async (currentPage: number, search = searchTerm, type = filterType, level = filterLevel) => {
    ...
    try {
      ...
      const [listRes, countRes] = await Promise.allSettled([
        bankAccountApi.getTransitBankAccounts({ page: currentPage, limit: itemsPerPage, ...filterPayload }),
        bankAccountApi.getTransitBankAccountCount({ ...filterPayload }),
      ])
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center justify-between mb-5">
        ...
        <Link
          href="/business-setup/transit-bank-account/create"
                  ...
                  return (
                    <tr
                      ...
                      onClick={() => {
                        ...
                        if (next) sessionStorage.setItem('transitbankaccount_highlight', next)
                        else sessionStorage.removeItem('transitbankaccount_highlight')
                      }}
                      className={clsx(
                    ...
                    >
                      ...
                      <td className="px-4 py-3 border-b border-gray-100 whitespace-nowrap">
                        <Link
                          href={`/business-setup/transit-bank-account/${account.accountId}/update`}
                      ...
                      <td className="pl-24 pr-4 py-3 border-b border-gray-100" onClick={e => e.stopPropagation()}>
                        <div className="relative" ref={el => { menuRefs.current[account.accountId] = el }}>
                          ...
                          {openMenuId === account.accountId && (
                            <div className="fixed w-56 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-[9999]"
...
export default function TransitBankAccountPage() {
  return (
    <Suspense fallback={
