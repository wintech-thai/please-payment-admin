...
function RegisterAgentContent() {
  ...
  const isLineApi = searchParams.get('type') === 'line-api'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLineApi) {
        const res = await agentApi.addLineApiAgent({
          Code: code.trim(),
        ...
        const data = res.data as any
        agentId = data?.agent?.agentId ?? data?.agentId
        toast.success(m.createLineApiSuccess)
      } else {
      ...
      toast.error(err instanceof Error ? err.message : (isLineApi ? m.createLineApiFailed : m.createFailed))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center gap-3 mb-6">
        <button onClick={() => guardNavigation(() => router.push('/setting/agent'))}
          className="p-2 rounded-lg text-gray-500 hover:bg-gray-200 transition-colors">
          <ChevronLeft className="w-5 h-5" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {isLineApi ? m.registerLineApiTitle : m.registerTitle}
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            {isLineApi ? m.registerLineApiSubtitle : m.registerSubtitle}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar flex flex-col gap-2 pb-2">
          ...
          {/* Line Api Config — only for line-api type */}
          {isLineApi && (
            <div className="flex-none bg-white rounded-xl shadow-sm border border-gray-100 px-7 py-4">
              <div className="flex items-center gap-2.5 mb-3">
        ...
        <div className="flex-none flex justify-end gap-3 py-4 border-t border-gray-100">
          <button type="button" onClick={() => guardNavigation(() => router.push('/setting/agent'))}
