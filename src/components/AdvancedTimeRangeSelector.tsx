...
export function AdvancedTimeRangeSelector({ value, onChange, disabled, className }: Props) {
  ...
  useEffect(() => {
    ...
    if (value.type === 'absolute' && value.start && value.end) {
      setFromStr(toLocalStr(value.start))
      setToStr(toLocalStr(value.end))
    } else {
      setFromStr(toLocalStr(calcRelativeStart(value.value || '24h')))
      setToStr(toLocalStr(nowSec))
    }
  ...
  const handleApplyAbsolute = () => {
    ...
    onChange({
      ...
      label: `${fmtAbsLabel(start)} → ${fmtAbsLabel(end)}`,
    })
    setIsOpen(false)
  }
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-1.5">
                {filteredRanges.map(range => (
                  <button
                    ...
                    className={clsx(
                      'w-full text-left px-3 py-2 text-sm rounded-lg transition-colors flex items-center justify-between gap-2',
