...
export function AuditLogHistogram({ data, totalHits, interval, maxDocCount, dict }: AuditLogHistogramProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-5 pt-4 pb-10 flex flex-col relative z-30 select-none overflow-visible" style={{ height: '160px' }}>

      {/* Header */}
      <div className="flex items-center justify-between mb-3 flex-none">
        <div className="text-sm font-bold text-gray-900">
          {totalHits.toLocaleString()}
          <span className="text-gray-400 font-normal text-xs ml-1.5">hits</span>
        </div>
        {data.length > 0 && (
          <div className="text-[10px] font-mono text-primary-600 bg-primary-50 px-2 py-0.5 rounded border border-primary-100">
            Interval: {interval}
          </div>
        )}
      </div>
      ...
      {/* Chart */}
      <div className={clsx('flex-1 flex items-end min-h-0', data.length > 80 ? 'gap-0' : 'gap-[2px]')}>
        {data.length === 0 ? (
          ...
          return (
            <div key={i} className="flex-1 min-w-0 h-full flex flex-col justify-end group relative cursor-pointer">
              ...
              <div className="absolute inset-y-0 inset-x-[-1px] bg-primary-50 hidden group-hover:block z-0 pointer-events-none rounded-t-sm" />

              <div className="w-full flex flex-col justify-end h-full z-10 relative opacity-80 group-hover:opacity-100 transition-opacity">
                {allBuckets.length === 0 && bucket.doc_count > 0 ? (
                  <div
                    style={{ height: `${(bucket.doc_count / (maxDocCount || 1)) * 100}%`, backgroundColor: '#3b82f6' }}
                    className="w-full"
                  />
                  ...
                  <div
                    ...
                    className={clsx('w-full border-b border-white/40 last:border-0', data.length > 100 && 'border-0')}
                  />
                ))}
              </div>
