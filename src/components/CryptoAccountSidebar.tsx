...
export default function CryptoAccountSidebar() {
  const { t } = useLang()
  ...
  const menuItems = [
    {
      ...
      label: t.nav.payInCryptoAccountSub,
      icon: (
    ...
    {
      href: '/business-setup/transit-crypto-account',
      label: t.nav.transitCryptoAccountSub,
      icon: (
        <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
    ...
    {
      href: '/business-setup/pay-out-crypto-account',
      label: t.nav.payOutCryptoAccountSub,
      icon: (
        <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
    ...
    >
      ...
      {!collapsed && (
        <div className="px-4 pt-5 pb-3">
          <p className="text-xs font-semibold text-white/40 uppercase tracking-widest">{t.nav.cryptoAccounts}</p>
        </div>
      )}
