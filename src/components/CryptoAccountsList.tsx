...
function CryptoAccountsContent({ accountType }: { accountType: AccountType }) {
  const { t } = useLang()
  const m = t.cryptoAccount
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const highlightIdParam = searchParams.get('highlight')
  const highlightKey = `cryptoaccount_${accountType.toLowerCase()}_highlight`

  const title = accountType === 'PayIn' ? m.titlePayIn : accountType === 'Transit' ? m.titleTransit : m.titlePayOut
  const createHref = `${pathname}/create`

  const [accounts, setAccounts] = useState<CurrencyAccountItem[]>([])
  ...
  const [selectedRowId, setSelectedRowId] = useState<string | null>(() => {
    ...
    if (typeof window !== 'undefined') return sessionStorage.getItem(highlightKey) ?? null
    return null
  })
  const [openMenuId, setOpenMenuId] = useState<string | null>(null)
  const [menuPos, setMenuPos] = useState<{ top?: number; bottom?: number; right: number } | null>(null)
  const [confirmDialog, setConfirmDialog] = useState<{ type: 'enable' | 'disable'; account: CurrencyAccountItem } | null>(null)
  ...
  const fetchAccounts = async (currentPage: number, search = searchTerm, level = filterLevel) => {
    setLoading(true)
    try {
      const filterPayload: Record<string, string> = { AccountType: accountType }
      if (search.trim()) filterPayload.FullTextSearch = search.trim()
      if (level) filterPayload.AccountLevel = level
      const [listRes, countRes] = await Promise.allSettled([
        cryptoAccountApi.getCurrencyAccounts({ page: currentPage, limit: itemsPerPage, ...filterPayload }),
        cryptoAccountApi.getCurrencyAccountCount({ ...filterPayload }),
      ])
      if (listRes.status === 'fulfilled') {
        const raw = listRes.value.data as any
        const list: any[] = Array.isArray(raw) ? raw : (raw?.currencyAccounts ?? raw?.CurrencyAccounts ?? [])
        setAccounts(list.map(item => ({
          ...item,
          accountId: item.id || item.currencyAccountId || item.CurrencyAccountId || item.accountId || item.AccountId || item.Id || '',
  ...
  useEffect(() => { fetchAccounts(page, searchTerm, filterLevel) }, [page, itemsPerPage, filterLevel, accountType])

  const handleSearch = () => { setPage(1); fetchAccounts(1, searchTerm, filterLevel) }

  const handleToggleActive = async (account: CurrencyAccountItem) => {
    const active = isAccountActive(account.status)
    const name = account.currency || account.accountId.slice(0, 8)
    try {
      if (active) {
        await cryptoAccountApi.disableCurrencyAccountById(account.accountId)
        toast.success(`"${name}" ${m.disabledSuccess}`)
      } else {
        await cryptoAccountApi.enableCurrencyAccountById(account.accountId)
  ...
  const handleDelete = async () => {
    ...
    try {
      if (deleteModal.bulk) {
        for (const id of selectedIds) await cryptoAccountApi.deleteCurrencyAccountById(id)
        toast.success(m.deletedBulk.replace('{count}', String(selectedIds.length)))
        setSelectedIds([])
      } else {
        await cryptoAccountApi.deleteCurrencyAccountById(deleteModal.accountId!)
  ...
  const cols = [
    '', // checkbox
    m.colCurrency, m.colAccountLevel, m.colLinkCount,
    m.colRange, m.colDailyLimit,
    ...
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      {confirmDialog && (
        <ConfirmDialog
          ...
          accountName={confirmDialog.account.currency || ''}
                ...
                accounts.map((account, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 border-b border-gray-100 text-sm text-gray-600 whitespace-nowrap">
                        {account.merchantLinkCount != null ? account.merchantLinkCount : '—'}
                      </td>
                      <td className="px-4 py-3 border-b border-gray-100 text-sm text-gray-700 whitespace-nowrap tabular-nums">
                        {formatRange(account.txMinAmount, account.txMaxAmount)}
                      </td>
                      <td className="px-4 py-3 border-b border-gray-100 text-sm text-gray-700 whitespace-nowrap tabular-nums">
                        {account.dailyTotalAmountLimit != null ? fmtNum(account.dailyTotalAmountLimit) : '—'}
                      </td>
                      <td className="px-4 py-3 border-b border-gray-100 whitespace-nowrap">
                          ...
                          {openMenuId === account.accountId && (
                            <div className="fixed w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-[9999]"
                              ...
                              <button
                                disabled
                                title={t.nav.comingSoon}
                                className="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm text-gray-400 cursor-not-allowed whitespace-nowrap"
                              >
...
export function CryptoAccountsList({ accountType }: { accountType: AccountType }) {
  return (
    <Suspense fallback={
      ...
      <CryptoAccountsContent accountType={accountType} />
    </Suspense>
  )
}
