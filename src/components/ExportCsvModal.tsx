...
export interface ExportCsvModalProps<T> {
  ...
  fetchPage: (params: ExportFetchParams, page: number, limit: number) => Promise<T[]>
...
export default function ExportCsvModal<T>({
  ...
  const [timeRange, setTimeRange] = useState<TimeRangeValue>(defaultTimeRange ?? { type: 'relative', value: '24h' })
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      for (let page = 1; page <= totalPages; page++) {
        const items = await fetchPage(params, page, PAGE_LIMIT)
        for (const item of items) rows.push(mapRow(item))
        setProgress({ done: Math.min(page * PAGE_LIMIT, total), total })
