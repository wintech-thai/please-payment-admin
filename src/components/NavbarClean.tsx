        ...
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          <Link href="/login" className="flex items-center gap-3 group">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120" fill="none" className="w-8 h-8 flex-shrink-0">
              ...
              <text x="60" y="89" textAnchor="middle" fill="#1e3a8a" fontSize="42" fontWeight="bold" fontFamily="Arial, sans-serif">฿</text>
            </svg>
            <div>
              <p className="font-bold text-white text-sm tracking-wide leading-none">PLEASE-PAYMENT</p>
              <p className="text-blue-200 text-xs leading-none mt-0.5">Admin</p>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-1 bg-white/10 rounded-lg p-0.5">
            {(['th', 'en'] as Lang[]).map(l => (
              <button
                key={l}
                onClick={() => setLang(l)}
                className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${lang === l ? 'bg-white/30 text-white' : 'text-blue-300 hover:text-white'}`}
              >
                {l === 'th' ? 'TH' : 'EN'}
              </button>
            ))}
