...
export function BlacklistProvider({ children }: { children: ReactNode }) {
  ...
  const check = useCallback(async () => {
    try {
      const res = await client.get('/api/Organization/org/global/action/GetIpPolicyStatus')
      ...
      // Fail-open: never block the admin portal just because the check itself failed.
      setStatus(s => ({ ...s, loading: false }))
    }
  }, [])

  // Re-check on first load and whenever the admin navigates to a different page.
  useEffect(() => {
    check()
  }, [check, pathname])
